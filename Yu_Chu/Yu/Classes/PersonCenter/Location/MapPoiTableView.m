...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.textColor = JQXXXLZH272727CLOLR;
    cell.textLabel.font = [UIFont systemFontOfSize:14];
    cell.detailTextLabel.textColor = JQXXXLZHB7B7B7CLOLR;
    cell.detailTextLabel.font = [UIFont systemFontOfSize:10];

    
    if (indexPath.row == 0) {
        cell.textLabel.textColor = JQXXXLZHFF2D4BCLOLR;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AMapPOI *point = _searchPoiArray[indexPath.row];
     [[self getParentviewController].navigationController popViewControllerAnimated:YES];
     [[NSNotificationCenter defaultCenter] postNotificationName:@"slectPoint" object:point];
...
- (void)onReGeocodeSearchDone:(AMapReGeocodeSearchRequest *)request response:(AMapReGeocodeSearchResponse *)response
{
    if (response.regeocode != nil) {
        ...
        _selectedPoi.city = component.city;
        _selectedPoi.province = component.province;
